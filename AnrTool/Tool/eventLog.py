...
class AnrLine(LogLine):
    ...
    def isAnrLine(self, packageName: str = 'com.android.systemui'):
        ...
        if match:
            self.packageName = match.group(3).strip()
...
class EventAnr():
    ...
    def parser(self, msg: str, allAnr:Anr):
        match = re.match(EventAnr.pid_pattern, msg)
        if match:
        ...
        match = re.match(EventAnr.reason_pattern, msg)
        if match:
            self.anr.anrReason = match.group(1).strip()
            self.parseReason(self.anr.anrReason)
            return
        match = re.match(EventAnr.cpu_pattern, msg)
        if match:
    ...
    def parseReason(self, reson: str):
        match = re.match(EventAnr.broadcast_pattern, reson)
        if match:
            self.anr.setAnrBroadcast(match.group(1))
            return
        match = re.match(EventAnr.service_pattern, reson)
        if match:
            self.anr.setAnrService(match.group(1))
            return
        match = re.match(EventAnr.input_pattern, reson)
        if match:
                ...
                while True:
                    ...
                    if not line:
                        ...
                        if temp.isAnrLine(self.packageName):
                            anr = Anr(temp)
                            eventAnr = EventAnr(temp, anr)
                            anr.eventAnr = eventAnr
                            eventAnr.addLine(temp, self.allAnr)
